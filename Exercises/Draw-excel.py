...
# 读取文档的每一行，至列表，如果不是双电源的问题，直接 for line in file.readlines():
...
F51FSwitch = workbook.add_sheet(
    'F51FSwitch', cell_overwrite_ok=True)  # 创建表格的某一分页
...
# 合并单元格(1,11为行1到11行 0,0为列0到0)，填入内容
F51FSwitch.write_merge(1, 11, 0, 0, 'QCMC-F5-1FA', style3)
# F51FSwitch.write_merge(1,10,0,1,'QCMC-F3-1FA')#合并0到1列，1到10行
# 添加style3的样式，只能填写一个，所以初始化样式的时候根据需求添加多个属性。
F51FSwitch.write_merge(1, 11, 1, 1, '10.20.5.1', style3)
