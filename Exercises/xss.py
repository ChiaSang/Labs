...
print(Fore.GREEN + "XSS DETECTOR" + Fore.RED +
      "  by HackLine" + Style.RESET_ALL)
time.sleep(2)
print()
...
if pog == "g":
    ...
    try:
        ...
        print()
        print("The file " + Fore.RED + repertoirepayload +
              Style.RESET_ALL + " doesn't exist, try again !")
        sys.exit(0)
    print()
    ...
    for line in f:
        ...
        if line in requests.get(site + line, headers=header).text:
            ...
            print(Fore.RED + "The payload" + str(l) +
                  " does not trigger the XSS Filter." + Style.RESET_ALL)
            print()
            l += 1
elif pog == "p":
    try:
    ...
    for line in f:
        ...
        if line in requests.get(site + line, headers=header, data=data).text:
            print(Fore.RED + "XSS FOUND HERE:\n" + Style.RESET_ALL)
            print(requests.get(site + line, headers=header,
                               data=data.url + " Post DATA = " + data + "\n"))
        else:
            print(Fore.RED + "The payload" + str(l) +
                  " does not trigger the XSS Filter." + Style.RESET_ALL)
            print()
